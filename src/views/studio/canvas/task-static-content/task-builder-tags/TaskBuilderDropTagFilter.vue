<template>
    <div
        ...
        <drag :list="fields" class="flex flex-wrap gap-1" :group="{ name: 'items', pull: 'clone' }" pull="clone"
            @change="updateFields">
            <div v-for="(field, i) in fields" :key="i" class="flex gap-1">
                <n-tag v-if="field && i > 0" @click="interchangeAndOr(field)">
                    {{ $t(field.andOr || 'and') }}
                </n-tag>
                <v-tag v-if="field" :key="i" :field="field" @click="$emit('choose', field)" />
            </div>
        </drag>
    </div>
</template>
