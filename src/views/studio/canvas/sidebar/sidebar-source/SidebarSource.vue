        ...
        <div class="my-3 flex grow flex-col items-stretch overflow-hidden">
            <div class="mx-3">
                <n-collapse @item-header-click="loadMetadata">
                    <n-collapse-item v-for="source in sourceList" :key="source.sourceName" :title="source.sourceName"
                        :name="source.sourceName">
                        <div class="mx-3">
                            <n-input v-model:value="searchTerm[source.sourceName]" :placeholder="$t('search')"
                                clearable />
                            <n-scrollbar style="max-height: 200px">
                                <vue-draggable :key="source.items?.length"
                                    :model-value="filterBy(source.items, 'tableName', searchTerm[source.sourceName])"
                                    :sort="false" :group="{ name: 'sources', pull: 'clone', put: false }"
                                    class="drag-table">
                                    <div v-for="tb of filterBy(source.items, 'tableName', searchTerm[source.sourceName])"
                                        :key="tb.tableName"
                                        class="flex items-center border-b py-1 last-of-type:border-0">
                                        <div class="flex-grow truncate">{{ tb.tableName }}</div>
                                        <div v-if="tb.sourceType === 'bucket'" class="drag-hide">
...
import type { FieldType, SourceType, TableType } from '@gaio/shared/types'
