        ...
        <template #content>
            <div class="task-run-flow flex flex-col items-center justify-center gap-1 overflow-auto">
                <div class="control flex w-full flex-col gap-1">
                    ...
                    <n-select v-model:value="localTask.flowId" :options="flowListOptions"
                        :placeholder="$t('selectFlow')" @update:value="defineLabel" />
                </div>
                <div class="control flex w-full flex-col gap-1">
                    <label class="control-label">
                        {{ $t('task') }}
                ...
                <div class="flex w-full flex-col items-start justify-start gap-1">
                    <label class="font-semibold text-neutral-500" for="max-repeat">
                        {{ $t('maxRepeat') }}
                    </label>
                    <n-input-number v-model:value="localTask.loopSize" :placeholder="$t('runFlowLoopWarning')" clearable
                        class="w-full" />
                </div>
            </div>
            <div class="flex justify-end bg-paper-100 px-4 py-2 dark:bg-carbon-200">
                <n-button type="primary" :disabled="!isValid" @click="save()">
                    {{ $t('save') }}
                </n-button>
            </div>
        </template>
