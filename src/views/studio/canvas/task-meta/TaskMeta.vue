<template>
    <g-dialog @close="$emit('close')">
        ...
        <template #tabs>
            <div class="task-meta">
                <n-tabs pane-class="g-bg-2" size="small" type="line">
                    <n-tab-pane name="1" :tab="$t('general')">
                        <div class="px-5 py-3">
                            ...
                            <div class="control">
                                <div>
                                    ...
                                    <n-input v-model:value="localMeta.description" type="textarea" />
                                </div>
                            </div>
                        </div>
                    </n-tab-pane>
                    <n-tab-pane v-if="localMeta.type === 'power'" name="2" :tab="$t('fields')">
                        <div class="px-5 py-4">
                            <task-meta-fields :local-meta="localMeta" />
                        </div>
                    </n-tab-pane>
                    <n-tab-pane v-if="localMeta.type === 'insights'" name="3" :tab="$t('options')">
                        <div class="px-5 py-4">
        ...
        <template #footer>
            <div class="flex justify-between gap-2">
                <n-switch v-model:value="localMeta.status" checked-value="active" unchecked-value="inactive">
                    <template #checked>
                        {{ $t('active') }}
                    </template>
                    <template #unchecked>
                ...
                <n-button type="primary" :loading="loading" @click="saveMeta">
                    {{ $t('save') }}
                </n-button>
            </div>
        </template>
