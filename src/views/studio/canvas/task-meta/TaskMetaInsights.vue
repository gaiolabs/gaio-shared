<template>
    <div class="task-task-insights">
        ...
        <div v-else-if="localMeta?.options?.insights">
            ...
            <n-tabs type="line" class="inner-tabs">
                <n-tab-pane :tab="$t('general')" name="general">
                    <div class="g-card control p-2">
                        <div class="control">
                            <div class="control-label">{{ $t('date', 2) }}</div>
                            <g-select-column v-model="localMeta.options.insights.date" :table-name="localMeta.tableName"
                                :data-type-filter="['date', 'datetime']" multiple />
                        </div>
                        <div class="control">
                            <div class="control-label">{{ $t('dimension', 2) }}</div>
                            <g-select-column v-model="localMeta.options.insights.dimension" multiple
                                :table-name="localMeta.tableName" :data-type-filter="['text', 'uuid']" />
                        </div>
                        <div class="control">
                            ...
                            <n-select v-model:value="localMeta.options.insights.type" :options="[
                                {
                                    label: $t('field'),
                                    value: 'field'
                                },
                        ...
                        <n-table v-if="localMeta.options.insights.type === 'computed'">
                            <thead>
                                ...
                                <th>{{ $t('name') }}</th>
                                <th style="width: 35px">
                                    <g-icon name="add" @click="addComputed" />
                                </th>
                            </thead>
                            <tbody>
                                <tr v-for="(item, index) of localMeta.options.insights.list" :key="index">
                                    <td>{{ index + 1 }}</td>
                                    <td>
                                        <g-select-column v-model="item.left" :table-name="localMeta.tableName"
                                            :data-type-filter="['decimal', 'integer']" />
                                    </td>
                                    <td class="el-text-center">
                                        <n-select v-model:value="item.operator" :options="[
                                            {
                                                label: '/',
                                                value: '/'
                                            },
                                    ...
                                    <td>
                                        <g-select-column v-model="item.right" :table-name="localMeta.tableName"
                                            :data-type-filter="['decimal', 'integer']" />
                                    </td>
                                    <td>
                                        <n-input v-model:value="item.label" v-alpha />
                                    </td>
                                    <td>
                                        <g-icon v-if="localMeta.options.insights.list.length > 1" name="delete"
                                            @click="removeComputed(index)" />
                                    </td>
                                </tr>
                            </tbody>
                        </n-table>
                        <div v-if="localMeta.options.insights.type === 'field'" class="control">
                            <div class="control-label">{{ $t('measure', 2) }}</div>
                            <g-select-column v-model="localMeta.options.insights.measure" multiple
                                :table-name="localMeta.tableName" :data-type-filter="['decimal', 'integer']" />
                        </div>
                    </div>
                </n-tab-pane>
                <n-tab-pane :tab="$t('advanced')" name="advanced">
                    <div class="g-card px-3 py-2">
                        <!-- DEFAULTS GENERAL -->
                        <div class="control">
                            {{ $t('default') }} {{ $t('period') }}
                            <n-select v-model:value="localMeta.options.period" :options="[
                                {
                                    label: $t('daily'),
                                    value: 'daily'
                                },
                        ...
                        <div class="control">
                            {{ $t('growthPercentage') }} (%)
                            <n-input-number v-model:value="temporaryPercentage" :min="1" :max="100" :step="1" />
                        </div>

                        <div class="control">
                            {{ $t('numeric') }}/{{ $t('decimalSize') }}
                            <n-input-number v-model:value="localMeta.options.numberFormat.formatDecimalSize" :min="1"
                                :step="1" :max="8" />
                        </div>
                        <div class="control">
                            {{ $t('percent') }}/{{ $t('decimalSize') }}
                            <n-input-number v-model:value="localMeta.options.percentFormat.formatDecimalSize" :min="1"
                                :step="1" :max="8" />
                        </div>
                        <div class="control">
                            {{ $t('separator') }}
                            <div class="control-top">
                                <n-switch v-model:value="separator" active-value="dotComma" inactive-value="commaDot"
                        ...
                        <!--SCHEDULE INIT-->
                        <div v-if="showInsightsButton" class="control-top d-flex w-100">
                            <div class="g-card">
                                <div class="d-flex align-items-center justify-content-between">
                                    <n-switch v-model:value="localMeta.options.schedule"
                                        :active-text="$t('lang.SCHEDULE_INSIGHTS_ALERTS')" />

                                    <div v-if="localMeta.options.schedule && localMeta.id">
                                        <n-tooltip :persistent="false" placement="top">
                                            <template #trigger>
                                                <n-button type="primary" :loading="generateInsightLoading"
                                                    @click="save()">
                                                    {{ $t('lang.SAVE_AND_RUN') }}
                                                </n-button>
                                            </template>
