        ...
        <div class="flex-between flex items-center gap-2 border-b border-paper-400 p-1 py-2 dark:border-carbon-400">
            <template v-if="showTab === 'main'">
                <div class="flex-vertical-center gap-3">
                    <div v-for="report of reportTypes" :key="report.label">
                        <n-button size="tiny" text @click="emit('change', report)">
                            <template #icon>
                ...
                <div class="flex-vertical-center gap-3">
                    <div v-for="action of actions" :key="action.label" class="flex-vertical-center gap-2">
                        <n-button size="tiny" text @click="doAction(action.action)">
                            <template #icon>
                ...
                <div class="flex w-full gap-2">
                    <div class="flex-grow">
                        <n-input-group size="tiny">
                            ...
                            <n-input v-model:value="usePowerStore().selectedView.label" size="tiny"
                                :placeholder="$t('typeHere')" :readonly="loading" />
                        </n-input-group>
                    </div>
                    <div>
                        <n-button size="tiny" type="primary" :disabled="!usePowerStore().selectedView.label"
                            :loading="loading" @click="saveView">
                            {{ $t('save') }}
                        </n-button>
                    </div>
                </div>
