            ...
            <div class="flex w-full items-center justify-between">
                <div class="flex items-center gap-2">
                    <div v-if="localApp?.options?.icon">
                        <g-app-icon :name="localApp.options.icon" :size="18" :color="localApp.options.color" />
                    </div>
        ...
        <template #tabs>
            <n-tabs pane-class="bg-elevation-1" size="small" type="line" :default-value="currentTab">
                <n-tab-pane name="general" :tab="$t('general')" display-directive="show:lazy">
                    <app-control-general v-if="localApp.options" :local-app="localApp" />
                </n-tab-pane>
                <n-tab-pane v-if="localApp.appId" name="process" :tab="$t('process')" display-directive="show:lazy">
                    <app-control-flow :local-app="localApp" />
                </n-tab-pane>
                <n-tab-pane v-if="localApp.appId" name="portal" :tab="$t('portal')"
                    display-directive="show:lazy"></n-tab-pane>
                <n-tab-pane v-if="localApp.appId" name="apiKeys" :tab="$t('apiKeys')"
                    display-directive="show:lazy"></n-tab-pane>
            </n-tabs>
