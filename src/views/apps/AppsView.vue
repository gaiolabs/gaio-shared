<template>
	...
	>
		<div
			id="apps-wrapper"
			class="@container w-full max-w-[1440px] ultrawide:max-w-[1920px] overflow-hidden p-8 flex flex-col gap-8"
		>
			<header class="flex items-center justify-between">
				<h3 class="inline-flex gap-2 items-center text-xl">
					<IconComponent
						class="h-6 w-6 mb-[2px]"
						name="Apps"
					/>
					<span>
						{{ $t('dataApps') }}
			...
			>
				...
				<div
					...
					class="w-full rounded-2xl g-base p-4"
				>
			...
			<section
				id="all-apps"
				class="mb-2 mt-2 flex flex-col gap-4"
			>
				<header class="flex items-center justify-between">
					...
					<div
						class="g-base flex p-2 rounded-2xl gap-2"
				...
				<template v-if="listType === 'grid'">
					<div
						class="grid g-base dark:bg-white/[7.5%] grid-cols-1 sm:grid-cols-2 md:grid-cols-3 ultrawide:grid-cols-6 p-4 rounded-3xl gap-4 lg:grid-cols-4"
					>
						<template v-if="loading">
							<div
								v-for="n in 8"
