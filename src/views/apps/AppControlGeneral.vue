<template>
    <div v-if="localApp && localApp.options" class="app-control-general">
        <div v-if="localApp.options" class="app-control bg-elevation-1 p-3 px-4">
            ...
            <div class="mt-2">
                ...
                <n-select v-model:value="localApp.repoId" :disabled="!!localApp.appId" :options="repoList" />
            </div>
            <div class="mt-2 flex gap-2">
                ...
                <div class="w-[70px]">
                    <div>{{ $t('color') }}</div>
                    <n-color-picker v-model:value="localApp.options.color" :render-label="() => ''" :modes="['hex']" />
                </div>
                <div class="w-[55px]">
                    <div>{{ $t('icon') }}</div>
                    <n-button type="primary" @click="showIconControl = true">
                        <template #icon>
            ...
            <div class="mt-2">
                {{ $t('dashboardLink') }}
                <n-list v-if="localApp.appToken" size="small" bordered>
                    <n-list-item>
                        {{ appUrl }}
                    </n-list-item>
                </n-list>
        ...
        <g-icon-finder v-if="showIconControl" @close="showIconControl = false"
