...
const data = () => {
	const data: HeatmapDataItemOption = list.map((item) => {
		return [item[columnName(dimensions.value.first)], item[columnName(measures.value.first)]]
	})
	return data
}

const range = () => {
	const sortedData = data().sort((a: string, b: string) => new Date(a[0]).getTime() - new Date(b[0]).getTime())

	let oldest: string = sortedData[0][0]
	let newest: string = sortedData[sortedData.length - 1][0]
