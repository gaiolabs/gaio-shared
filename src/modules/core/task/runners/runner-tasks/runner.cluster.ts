...
import type { ClusterTaskType } from '@gaio/types'
...
export default async <T extends ClusterTaskType>(taskData: T) => {
    const autoPath = join(__dirname, '..', '..', '..', '..', 'cluster')
    ...
    await runCluster(taskData, autoPath)
...
const runCluster = async (taskData, autoPath) => {
    try {
        return await shell('run cluster executable', 'python3', [`${autoPath}/${taskData.id}.py`])
    } catch (e) {
...
const importToBucket = async (taskData, autoPath) => {
    ...
    if (countCsvBeforeImport && countCsvBeforeImport['count'] === '0') {
        ...
        const repoData = await RepoModel.getRepoCredentials(taskData.repoId, taskData.appId)
...
const createLocalData = async (taskData, autoPath) => {
    ...
    const repo = await RepoModel.getRepoCredentials(taskData.repoId, taskData.appId)
...
const createExecutable = async (taskData: ClusterTaskType, autoPath) => {
