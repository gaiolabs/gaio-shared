...
export class UpdateService {
    ...
    private static getValueFromParameter(value) {
    ...
    public build(taskData: CommonBuilderTaskType, params: ParamType[], builder: Knex) {
        ...
        schema.update.forEach((field) => {
            if (field.valueType === 'computed') {
                ...
                if (p) {
                    builder.update(field.columnName, UpdateService.castValue(field, p.paramValue, k))
                }
            } else {
                builder.update(field.columnName, UpdateService.castValue(field, field.value, k))
            }
        })
    ...
    private getComputedContent(params: ParamType[], builder: Knex, field: FieldType, k: Knex) {
        return builder.update(field.columnName, k.raw(`${this.template.render(field.content, params)}`))
    }

    static isNumeric(field: FieldType) {
        return [
            'Int',
            'Int8',
            'Int16',
    ...
    // static transformValue(field: FieldType, value: GenericValueType) {
    ...
    //     }
    //
    //     param.paramValue = UpdateService.transformValue(field, param.paramValue) as string
    //
