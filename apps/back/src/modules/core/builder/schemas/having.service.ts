...
// need to make it with having

export class HavingService {
    template = new TemplateService()

    static transformValue(field: FieldType, value: unknown) {
        if (value === 'all_') return 'all_'
        return (
            HavingService.isNumeric(field) ?
            ...
            if (isDateLiteral(field.value)) {
                ...
                switch (field.valueType) {
                    ...
                    case 'parameter':
                        ...
                        break
                    default:
                        value = HavingService.transformValue(field, field.value).toString()
                        if (field.extraValue) {
                            extraValue = HavingService.transformValue(field, field.extraValue)
                        }
                        break
                }
            }
    ...
    getValueFromParameter(parameters: ParamType[], field: FieldType, type: string) {
        ...
        if (!(isArray(param.paramValue) && param.paramValue[0] === 'all_')) {
            if (
                ...
                param.paramValue = HavingService.transformValue(field, param.paramValue) as string
            }
        }
