...
export function useTree(taskData, discoveryTags, metadataFields, currentFilter) {
    const { t } = useI18n()
    ...
    const loadData = async (task, schema, limit) => {
        console.log('scheam', schema)
        const data = useApi().post('api/table/rows', {
            body: {
                taskData: {
                    ...task,
                    schema: {
    ...
    async function prepare() {
        ...
        resultData.infoStats = infoStats
    ...
    function alwaysSetFrequencyFieldToTheEnd() {
        task.schema.select = task.schema.select.sort((a, b) => {
            if (a.alias === 'freq') {
                return 1
            }
            if (b.alias === 'freq') {
    ...
    async function prepareTree() {
        if (nMeasure.value > 0 && !nDate.value && !nDimension.value) {
            ...
            console.log('.measureList.value', measureList.value)
            schema.sort = [
                {
                    order: 'desc',
                    ...measureList.value[0]
            ...
            if (list.length > 1) {
                ...
                if (canBeANumber(top)) {
                    ...
                    schema.sort = schema.select
                        .filter(
                            (o) =>
                                o.dataType?.includes('Int') ||
                                o.dataType?.includes('Float') ||
                                ['count', 'sum', 'avg'].includes(o.type)
                        )
                        .map((o) => {
                            return {
                                ...o,
    ...
    const listData = async (task, schema, limit, offset = undefined) => {
    ...
    const nMeasure = computed(() => {
        return measureList.value.length
    })

    const nDimension = computed(() => {
        return dimensionList.value.length
    })

    const nDate = computed(() => {
        return dateList.value.length
    })
